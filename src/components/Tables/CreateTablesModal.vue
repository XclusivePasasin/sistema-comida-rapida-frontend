        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
            ...
            <form @submit.prevent="submitForm">
                ...
                <div class="flex justify-end">
                    ...
                    <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white py-2 px-4 rounded-xl" :disabled="isSubmitting" >
                        Create Table
                    </button>
                </div>
            </form>
...
export default {
    ...
    methods: {
        ...
        async checkTableExists(tableNumber) {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/tables/check-existence?number=${encodeURIComponent(tableNumber)}`);
        ...
        async submitForm() {
            ...
            if (!this.errors.table_number) {
                ...
                if (exists) {
                    this.errors.table_number = "Table Number already exists. Please choose another.";
                    this.isSubmitting = false;
                    return;
                }

                try {
                    const response = await axios.post("http://127.0.0.1:8000/api/tables/create", {
                        table_number: this.form.table_number
                    });
                    console.log(response.data);
                    this.$emit("create-tables", this.form);
                    this.clearForm();
                    this.clearErrors();
                    this.closeModal();
                } catch (error) {
